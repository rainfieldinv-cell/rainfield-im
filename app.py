...
def show_step_toc():
    ...
    with st.expander("💾 목차·배치 저장 / 📂 불러오기 (내 PC — 안 지워지는 백업)", expanded=False):
        st.markdown(
            "**왜 필요한가요?** 목차·페이지 배치는 보통 자동 저장되지만, 프로그램이 "
            "업데이트되면 드물게 초기화될 수 있어요. 아래에서 **내 PC에 파일로 저장**해두면 "
            "그럴 때도 그대로 되살릴 수 있어요.\n\n"
            "**이렇게 쓰세요**\n"
            "1. 목차·페이지 배치를 다 넣은 뒤 → **왼쪽 ‘💾 저장’** 클릭 "
            "(`목차배치_….json` 파일이 내 컴퓨터에 다운로드됩니다)\n"
            "2. 나중에 목차가 비어 보이면 → **오른쪽 ‘📂 불러오기’** 에 그 파일을 올리면 끝 "
            "(방금 저장한 목차·페이지가 그대로 복원됩니다)\n\n"
            "※ 파일은 지우지 말고 보관하세요. 문서(딜)마다 따로 저장해두면 좋아요.")
