...
STEP_NAMES = [
    ...
    "하이라이트 구성",    # 2단계 (Executive Summary 카드 3개 직접 입력)
    "하이라이트 완성본",  # 3단계 (완성 슬라이드 이미지 확인)
    "목차 구성",          # 4단계 (원본 페이지 보며 목차·소제목 작성)
    "배치 확인",          # 5단계 (배치한 원본 페이지를 순서대로 이미지로 확인·수정)
    "레이아웃 선택",      # 6단계
    "이미지 선택",        # 7단계 (PPT 생성·다운로드)
]
