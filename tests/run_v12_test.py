"""run_v12_test.py — 수정된 빌더로 v12 빌드(save_presentation) + idx16/중복파트 검사."""
...
OUTPUT   = os.path.join(ROOT, "test_ai_builders_v12.pptx")
...
def main():
    ...
    print(f"[v12] 중복파트: {'있음 ' + str(dup) if dup else '없음 (정상)'}")
    ...
    print(f"\n[v12 idx16] prst={g.get('prst')} adj1={adj.get('fmla') if adj is not None else '없음'}")
